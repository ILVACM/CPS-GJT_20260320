...
| **6. 安装 Python 依赖（uv sync）** | `uv sync` | 逐包解析并安装，最终 `Resolved N packages` | ⚠️ **PyTorch 不会从 PyPI 安装**（Jetson 专用 wheel 见步骤 7）；如步骤 6 卡死在 torch，请用 `uv sync --no-deps` 跳过 torch 后单独安装 |
...
| **8. 编译 proto 文件** | `uv run python -m grpc_tools.protoc \<br>    --proto_path=proto \<br>    --python_out=proto \<br>    --grpc_python_out=proto \<br>    proto/rebar_inference.proto` | `proto/rebar_inference_pb2.py` 与 `rebar_inference_pb2_grpc.py` 重新生成 | ⚠️ proto 更新后必须重编译；使用 `uv run` 而非裸 `python` 以调用虚拟环境内的 grpcio-tools |
...
| 现象 | 原因 | 解决办法 |
|------|------|----------|
| **`[自检] 失败 — 安全退出`** | 环境自检未通过 | 查看具体失败项：① `local_ip` 与本机网卡 IP 不一致？② 50051 端口被占用？③ CUDA 不可用？④ `model_path` 文件不存在？ |
...
- 一键部署脚本：`scripts/deploy_node_detect.sh`（Phase 4 生成）
