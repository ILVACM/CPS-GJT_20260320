...
from utils.common.constants import SegClass
from utils.measure.rebar_measure import MeasureResult
...
class UIRenderer:
    ...
    def _draw_degraded_banner(canvas: np.ndarray) -> None:
        """顶部黄色横幅：推理服务离线 → 降级模式（Inquiry Offline → DEGRADED MODE）。"""
        h, w = canvas.shape[:2]
        banner_h = 36
        text = "Inquiry Offline - DEGRADED MODE"
        font = cv2.FONT_HERSHEY_SIMPLEX
