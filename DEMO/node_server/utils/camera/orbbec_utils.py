...
# ******************************************************************************
"""Orbbec VideoFrame 帧格式转换工具。

来源：demo3 (OrbbecSDK-Linux-ARM64-python39/utils.py)，对应方案 F6。
直接复制到 camera 包内（K11：禁止跨目录 import，避免 from ..utils 不可靠）。

仅当 pyorbbecsdk 可用时才会被导入（由 orbbec_336l.py 懒导入控制）。
"""
...
def frame_to_bgr_image(frame: VideoFrame) -> Union[Optional[np.ndarray], Any]:
    ...
    支持格式：RGB / BGR / YUYV / MJPG / I420 / NV12 / NV21 / UYVY。
    来源：demo3 utils.py::frame_to_bgr_image（方案 F6，直接复制）。
    """
