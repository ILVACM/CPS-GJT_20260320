#!/bin/bash
# 检测节点 系统服务管理脚本（systemd）
# 对齐 Design-AI_detect.md §4 / §5 七段式主循环，以及 AGENTS.md §7.1（systemd unit 文件为 INI 格式例外）
#
# 用法：
...
#
# 与 node_server/scripts/service-manager.sh 的主要差异：
#   - Description 文案改为检测节点
#   - 新增读取 service.json 中可选字段 after / wanted_by（默认值保持与 node_server 行为一致，便于双节点脚本同源维护）
...
# node_detect 差异化字段：服务启动顺序依赖与开机启动目标（默认值保持与 node_server 行为同源）
SERVICE_AFTER="$(python3 -c "import json;print(json.load(open('$CONFIG_PATH')).get('after', ''))")"
SERVICE_WANTED_BY="$(python3 -c "import json;print(json.load(open('$CONFIG_PATH')).get('wanted_by', ''))")"
...
do_install() {
    ...
    # After / WantedBy 字段：默认值与 node_server 行为一致；可通过 service.json 字段覆盖
    cat > "$UNIT_FILE" <<EOF
[Unit]
Description=Rebar Detection Inference Node (钢筋直径测量设备 - 检测节点)
After=${SERVICE_AFTER:-network.target display-manager.service}
...
[Install]
WantedBy=${SERVICE_WANTED_BY:-graphical.target}
EOF
